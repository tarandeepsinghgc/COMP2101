...
function error-message {
  echo "Something got stuck! Sorry pal" >&2
}
...
function error-exit {
  echo "Something got stuck! Sorry pal! This may help! $1"
  exit
}
...
function cleanup {
  rm /tmp/*.$$
  [ $? -eq 0 ] && echo "Temporary files created by this script are deleted now. Storage space saved. Safe to exit now"
  exit
}
trap cleanup SIGINT
